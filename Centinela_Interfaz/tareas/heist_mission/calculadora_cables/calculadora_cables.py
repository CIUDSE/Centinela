...
class CalculadoraCables(Analizar_Cables):
    ...
    def __init__(self, reglas: Optional[Any] = None) -> None:
        ...
        self.COLORES_VALIDOS:list = ["rojo", "blanco", "azul", "amarillo", "negro"]
        self.cable_a_cortar:dict[int, str] = {} # Para almacenar el resultado del análisis y poder usarlo en otros métodos si se desea
        self.cantidad_cables:int = None # Para almacenar la cantidad de cables y poder usarla en otros métodos si se desea
    ...
    def analizar_panel(self, cables: List[str]) -> int|str:
        ...
        if self.cantidad_cables < 3 or self.cantidad_cables > 6:
            raise ValueError(f"Cantidad de cables inválida: {self.cantidad_cables}. El reglamento especifica que solo hay entre 3 y 6 cables.")
        
        # Revisar si no existe algún color que no sea válido
        if any(c not in self.COLORES_VALIDOS for c in cables):
            raise ValueError(f"Colores no válidos en la lista: {cables}. El reglamento solo permite: {', '.join(self.COLORES_VALIDOS)}")
        ...
        if self.cantidad_cables == 3:
                if "rojo" not in cables:
                    ...
                    self.cable_a_cortar = {len(cables) - cables[::-1].index("azul"), "azul"}
                else:
                    self.cable_a_cortar = {3, cables[2]}
        
        elif self.cantidad_cables == 4:
            if cables.count("rojo") > 1:
                self.cable_a_cortar = {len(cables) - cables[::-1].index("rojo"), "rojo"}
    ...
    def procesar_entrada(self, entrada:str) -> List[str]:
        ...
        if entrada:
            ...
            colores_incorrectos = [c for c in cables if c not in self.COLORES_VALIDOS]
            
            if colores_incorrectos:
                raise ValueError(f"Colores no válidos: {colores_incorrectos}. El reglamento solo permite: {', '.join(self.COLORES_VALIDOS)}")
            return cables
